...
@router.post("")
def transfer(from_wallet_id: int, to_wallet_id: int, amount: Decimal, db: Session = Depends(get_db)):
    transfer_between_wallets = create_transfer(db, from_wallet_id, to_wallet_id, amount)
    return {
